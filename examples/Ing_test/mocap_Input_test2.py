...
def move_box_limit(cm):
    ...
    if position_estimate[0] > BOX_LIMIT_XY:
        body_x_cmd = -max_vel
    elif position_estimate[0] < -BOX_LIMIT_XY:
        body_x_cmd = max_vel

    if position_estimate[1] > BOX_LIMIT_XY:
        body_y_cmd = max_vel
    elif position_estimate[1] < -BOX_LIMIT_XY:
        body_y_cmd = -max_vel

    if position_estimate[2] < BOX_LIMIT_MIN_Z:
        body_z_cmd = max_vel
    elif position_estimate[2] > BOX_LIMIT_MAX_Z:
        body_z_cmd = -max_vel

    print("Warning: Position limit exceeded. Hovering in place.")
    for _ in range(100):
        cm.send_hover_setpoint(body_x_cmd, body_y_cmd, 0, 1)
        time.sleep(0.01)

    # Once back inside the box, move to the center (0, 0, 1) and land
    for _ in range(200):
...
def run_sequence(cf):
    ...
    def send_ang(cm, roll, pitch, yawrate, z, duration):
        for _ in range(duration):
            if (abs(position_estimate[0]) > BOX_LIMIT_XY or abs(position_estimate[1]) > BOX_LIMIT_XY or position_estimate[2] < BOX_LIMIT_MIN_Z or position_estimate[2] > BOX_LIMIT_MAX_Z):
                move_box_limit(cm)
                return
            cm.send_zdistance_setpoint(roll, pitch, yawrate, z)
            time.sleep(0.01)

    def send_hover(cm, x, y, z, duration):
        for _ in range(duration):
            if (abs(position_estimate[0]) > BOX_LIMIT_XY or abs(position_estimate[1]) > BOX_LIMIT_XY or position_estimate[2] < BOX_LIMIT_MIN_Z or position_estimate[2] > BOX_LIMIT_MAX_Z):
                move_box_limit(cm)
                return
            cm.send_hover_setpoint(x, y, 0, z)
            time.sleep(0.01)

    def send_position(cm, x, y, z, yaw, duration):
        for _ in range(duration):
            if (abs(position_estimate[0]) > BOX_LIMIT_XY or abs(position_estimate[1]) > BOX_LIMIT_XY or position_estimate[2] < BOX_LIMIT_MIN_Z or position_estimate[2] > BOX_LIMIT_MAX_Z):
                move_box_limit(cm)
                return
            cm.send_position_setpoint(x, y, z, yaw)
            time.sleep(0.01)
    ...
    cf.param.set_value('flapper.motBiasRoll', 0)
    cf.param.set_value('flapper.servPitchNeutr', pitch_neutral)
    cf.param.set_value('flapper.servYawNeutr', yaw_neutral)
    ...
    # Initial hover command
    send_hover(cm, 0, 0, zdistance, 200)

    # Initial position hold
    send_position(cm, x, y, zdistance, yaw, 500)
    
    # Sequence
    send_ang(cm, 0,     0,     0,       zdistance, 500)
    send_ang(cm, roll,  pitch, yawrate, zdistance, 500)
    send_ang(cm, roll, -pitch, yawrate, zdistance, 500)
    send_ang(cm, 0,     0,     0,       zdistance, 500)

    # Final hover command
    send_position(cm, 0, 0, zdistance, 0, 500)
    
    # Landing
    send_position(cm, 0, 0, 0, 0, 50)
