...
def generate_pitch_commands(amplitude, times):
    """
    3, -2, 1, -1 pitch 명령을 시간에 따라 생성하는 함수.
    각 명령은 동일한 진폭을 가지고 주어진 시간에 맞춰 조정됩니다.
    """
    commands = [3, -2, 1, -1]  # 3, -2, 1, -1 명령 패턴
    pitch_commands = []

    for command, time_duration in zip(commands, times):
        # 명령 시간이 주어졌으므로 주어진 시간 동안 command의 값을 유지하는 명령을 생성
        pitch_values = np.linspace(0, command * amplitude, int(time_duration / 0.01))
        pitch_commands.extend(pitch_values)

    return pitch_commands
...
def run_sequence(cf):
    ...
    for _ in range(200):
        cm.send_position_setpoint(0.5, 0, zdistance, 0)
    ...
    for _ in range(200):  # 0.01초마다 명령 전송
        cm.send_longmanual_control(thrust, trim_amplitude, roll, yawrate)
        time.sleep(0.01)
    
    for i, freq in enumerate(freq_range):
        current_time = i * 0.01
        pitch = -amplitude * np.sin(freq * current_time)  + trim_amplitude
        cm.send_longmanual_control(thrust, pitch, roll, yawrate)
        time.sleep(0.01)
    
    for _ in range(120):
        cm.send_longmanual_control(thrust, pitch+amplitude, roll, yawrate)
        time.sleep(0.01)
    ...
    for _ in range(3000):
        #cm.send_position_setpoint(0, 0, zdistance, 0)
        #cm.send_zdistance_setpoint(-1, -5, 0, zdistance)
        cm.send_longmanual_control(thrust, trim_amplitude, roll, yawrate)
        time.sleep(0.01)
    '''
